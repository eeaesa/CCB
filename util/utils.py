...
#---------------2d----------------
def test_single_pixel(image, label, net, classes, patch_size=256):
    image = image.cpu().detach().numpy()
    label = label.cpu().detach().numpy()
    prediction = np.zeros_like(label)
    ...
    for i in range(classes):
        dice, jac = calculate_metric_percase_2d(prediction == i, label == i)
        dice_list.append(dice)
        jac_list.append(jac)
    PA = np.sum(prediction == label) / label.size

    hd95, asd = compute_hd95_asd_2d(prediction == 1, label == 1)

    return dice_list, jac_list, PA, hd95, asd

def calculate_metric_percase_2d(pred, gt):
